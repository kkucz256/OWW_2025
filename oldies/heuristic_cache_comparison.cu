...
int main(int argc, char* argv[]) {
    ...
    Timer th; std::vector<Pixel> res(w*h); 
    ...
    {
        Timer tb; for (int i=0; i<ITERS; i++) blurKernelGlobal<<<grd, blk>>>(ds, dd, w, h);
        cudaDeviceSynchronize(); cu_global.blur = tb.elapsed() / ITERS;

        Timer tr; rleKernel<<<(h+255)/256, 256>>>(dd, dr, w, h); cudaDeviceSynchronize(); cu_global.rle_time = tr.elapsed();
        cudaMemcpy(hr.data(), dr, h*sizeof(uint32_t), cudaMemcpyDeviceToHost);
        for (auto s : hr) cu_global.rle_size += s;

        Timer td2h; cudaMemcpy(res.data(), dd, sz, cudaMemcpyDeviceToHost); cu_global.d2h = td2h.elapsed();
...

    {
        ...
        ks.regs_per_thread = attr.numRegs; ks.shared_static = attr.sharedSizeBytes; ks.local_per_thread = attr.localSizeBytes; ks.max_threads_per_block = attr.maxThreadsPerBlock;

        Timer tb; for (int i=0; i<ITERS; i++) blurKernelShared<<<grd, blk>>>(ds, dd, w, h);
        cudaDeviceSynchronize(); cu_shared.blur = tb.elapsed() / ITERS;

        Timer tr; rleKernel<<<(h+255)/256, 256>>>(dd, dr, w, h); cudaDeviceSynchronize(); cu_shared.rle_time = tr.elapsed();
        cu_shared.rle_size = 0;
        cudaMemcpy(hr.data(), dr, h*sizeof(uint32_t), cudaMemcpyDeviceToHost);
        for (auto s : hr) cu_shared.rle_size += s;

        Timer td2h; cudaMemcpy(res.data(), dd, sz, cudaMemcpyDeviceToHost); cu_shared.d2h = td2h.elapsed();
    ...
    saveReport("performance_report_full.txt", w, h, ths, loadT, 0, st, mt, cu_global, cu_shared, ks);
