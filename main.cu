...
__global__ void blurKernel(const Pixel* src, Pixel* dst, int w, int h) {
    ...
    dst[y * w + x].r = (byte)(r / count);
    dst[y * w + x].g = (byte)(g / count);
    dst[y * w + x].b = (byte)(b / count);
}

class CudaProcessor {
public:
    static void applyBlur(const Image& src, Image& dst) {
        dst = src; 
        ...
        dim3 numBlocks((src.width + threadsPerBlock.x - 1) / threadsPerBlock.x,
                       (src.height + threadsPerBlock.y - 1) / threadsPerBlock.y);
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cout << "[1] Running Single Thread (CPU)...\n";
        ...
        std::cout << "[2] Running Multi Thread (CPU " << threads << " threads)...\n";
        ...
        std::cout << "[3] Running CUDA (GPU)...\n";
        auto tStart3 = std::chrono::high_resolution_clock::now();
        CudaProcessor::applyBlur(input, blurred);
        auto tEnd3 = std::chrono::high_resolution_clock::now();
        double time3 = std::chrono::duration<double, std::milli>(tEnd3 - tStart3).count();
        std::cout << "Time: " << time3 << " ms\n";

        std::cout << "--------------------------------\n";
        std::cout << "SPEEDUP CPU (Multi/Single): x" << std::fixed << std::setprecision(2) << (time1 / time2) << "\n";
        std::cout << "SPEEDUP GPU (CUDA/Single):  x" << std::fixed << std::setprecision(2) << (time1 / time3) << "\n";
        std::cout << "--------------------------------\n\n";
