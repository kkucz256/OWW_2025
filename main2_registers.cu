...
void saveReport(
    ...
    line();
    f << "[CUDA KERNEL STATS]\n";
    f << "Registers per thread: " << ks.regs_per_thread << "\n";
    f << "Shared memory (static): " << ks.shared_static << " B\n";
    f << "Local memory per thread: " << ks.local_per_thread << " B\n";
    f << "Max threads per block: " << ks.max_threads_per_block << "\n";
}
...
int main(int argc, char* argv[]) {
    ...
    saveReport("performance_shared_report_registers.txt",
               w, h, threads,
               loadT, saveT,
               st, mt, cu,
               ks);
