...
__global__ void blurKernelShared(const Pixel* __restrict__ src, Pixel* __restrict__ dst, int w, int h) {
    ...
    if (x < w && y < h) tile[sy][sx] = src[y * w + x];
...
int main(int argc, char* argv[]) {
    ...
    Image gpuOut = img;
    {
        ...
        cu.d2h = t_d2h.elapsed();

        cudaFree(ds);
        cudaFree(dd);
        cu.total = cu.prep.ctx_init + cu.prep.alloc_device + cu.h2d + (cu.blur * ITERS) + cu.d2h;
    }

    Timer tSave;
    stbi_write_png("final_result.png", w, h, 3, gpuOut.pixels.data(), w * 3);
    double saveT = tSave.elapsed();
    saveReport("performance_shared_report_registers.txt", w, h, threads, loadT, saveT, st, mt, cu, ks);
    return 0;
}